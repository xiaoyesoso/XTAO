"""XTAO SDK - Async client for the XTAO G4C Plan service.

The SDK wraps all REST endpoints exposed by the XTAO FastAPI server and
provides a Pythonic, type-safe interface. It reuses the Pydantic models from
``xtao.models`` so callers can pass model instances directly without dealing
with raw JSON.
...
Usage:
    ...
    async def main():
        client = XTAOClient(base_url="http://localhost:8000")
...
class XTAOClient:
    """Async client for the XTAO service.

    The client is the single entry point for all XTAO operations. Each method
    ...
    Args:
        base_url: Base URL of the XTAO server, e.g. ``http://localhost:8000``.
    ...
    async def __aenter__(self) -> "XTAOClient":
        return self
    ...
    async def _request(
        ...
        *,
        ...
        try:
            ...
            raise ConnectionError(f"Cannot connect to XTAO server: {exc}") from exc
    ...
    async def run_plan(
        ...
        Args:
            ...
            config: Optional :class:`~xtao.models.OrchestratorConfig` (or dict)
        ...
        Returns:
            :class:`~xtao.models.OrchestratorResult` as a dict.
        """
        payload = {
    ...
    async def run_tao(
        ...
        Args:
            user_input: User's goal or request.
            plan: Optional :class:`~xtao.models.Plan` for goal/context anchoring.
            candidate_actions: Full candidate action space (list of
                :class:`~xtao.models.ActionCandidate` or dicts); coarse-filtered
                inside the engine.
            max_loops: Maximum TAO inner loop rounds.
            max_time: Maximum execution time in seconds.

        Returns:
            :class:`~xtao.models.TAOResult` as a dict.
        """
        payload = {
            "user_input": user_input,
    ...
    async def tao_think(self, state: Any) -> dict[str, Any]:
        ...
        :class:`~xtao.models.TAOState` between calls.

        Args:
            state: Current :class:`~xtao.models.TAOState` (or dict).
    ...
    async def tao_act(
        ...
        Args:
            state: Current :class:`~xtao.models.TAOState` (or dict) carrying
                the candidate action space.
            action_name: Selected action name (must be in the candidate space).
            params: Optional action parameters.

        Returns:
            Dict with ``record`` (:class:`~xtao.models.ActionRecord`).
        """
        payload = {
    ...
    async def tao_observe(
        ...
        Args:
            state: Current :class:`~xtao.models.TAOState` (or dict).
            record: :class:`~xtao.models.ActionRecord` (or dict) to interpret.
            expectation: Optional expectation for the output.

        Returns:
            Dict with ``observation`` (:class:`~xtao.models.Observation`).
        """
        payload = {
            "state": _to_payload(state),
    ...
    async def verify_plan(self, plan: Any) -> dict[str, Any]:
        ...
        Args:
            plan: A :class:`~xtao.models.Plan` instance (or dict).
    ...
    async def trace_failure(
        ...
        Args:
            ...
            step_records: Optional list of :class:`~xtao.models.StepRecord`.
